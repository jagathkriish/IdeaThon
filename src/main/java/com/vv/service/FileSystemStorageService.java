...
public class FileSystemStorageService implements StorageService {
    ...
    @Override
    public void storeIdea(MultipartFile docFile,String docFileName,MultipartFile videoFile,String videoFileName) {
        try {
            if (docFile.isEmpty()) {
                throw new StorageException("Failed to store empty file " + docFile.getOriginalFilename());
            }
            
            Files.copy(docFile.getInputStream(), this.rootLocation.resolve(docFileName));
            
            if(!videoFileName.equals("NA")){
            ...
            throw new StorageException("Failed to store file " + docFile.getOriginalFilename(), e);
        }
    }
